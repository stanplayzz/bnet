...
constexpr auto invalid_v = SocketHandle{INVALID_SOCKET};
constexpr auto error_v = SocketHandle{SOCKET_ERROR};

inline void close(SocketHandle const s) { ::closesocket(s); }

inline auto send(SocketHandle const socket, std::span<std::byte const> data) -> std::int64_t {
	void const* erased = data.data();
	return std::size_t(::send(socket, static_cast<char const*>(erased), int(data.size()), 0));
}

inline auto receive(SocketHandle const socket, std::span<std::byte> buffer) -> std::int64_t {
	void const* erased = buffer.data();
	return std::size_t(::recv(socket, static_cast<char*>(erased), int(buffer.size()), 0));
}
